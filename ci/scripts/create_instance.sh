...
if [[ -z "${SSDG_FORK}" ]]; then
  echo "SSDG_FORK environment variable must be set for this script to work."
  exit 1
fi

if [[ -z "${SSDG_BRANCH}" ]]; then
  echo "SSDG_BRANCH environment variable must be set for this script to work."
  exit 1
fi
...
SANITIZED_SSDG_FORK=$(getSanitizedFork ${SSDG_FORK})
...
IMAGE_FAMILY_PREFIX="${SANITIZED_SSDG_FORK}-${SANITIZED_SSDG_BRANCH}-"
